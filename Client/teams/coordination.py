...
class CoordinationManager:
    ...
    def _prepare_helper_response(self, direction: int) -> Tuple[str, Optional[int]]:
        ...
        if direction in [1, 3, 5, 7]:
            estimated_distance = min(W, H) // 8
        elif direction in [2, 4, 6, 8]:
            estimated_distance = min(W, H) // 6
        else:
            estimated_distance = min(W, H) // 4

        eta_seconds = max(2, estimated_distance * 2)
    ...
    def send_incant_request(self):
        ...
        food_needed = int(estimated_duration / 20) + 8
