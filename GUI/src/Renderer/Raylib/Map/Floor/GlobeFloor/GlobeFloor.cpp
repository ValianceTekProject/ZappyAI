...
zappy::gui::raylib::GlobeFloor::GlobeFloor(size_t width, size_t height, size_t tileSize) :
    AFloor(width, height, tileSize),
    _radius(tileSize / 2)
{}

void zappy::gui::raylib::GlobeFloor::render() const
{
    int numLatitudeDivisions = std::max(4, (int)(M_PI * _radius / getTileSize()));
    int numLongitudeDivisions = std::max(8, (int)(2 * M_PI * _radius / getTileSize()));
